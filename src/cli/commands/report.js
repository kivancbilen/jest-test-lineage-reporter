...
async function reportCommand(options) {
  try {
    ...
    const fs = require("fs");
    const mutationResultsPath = path.join(
      process.cwd(),
    ...
    if (fs.existsSync(mutationResultsPath)) {
      try {
        mutationResults = JSON.parse(
          fs.readFileSync(mutationResultsPath, "utf8"),
        );
        info(
          `Loaded mutation results: ${chalk.cyan(mutationResults.totalMutations)} mutations tested`,
        );
      } catch (e) {
    ...
    // Load full test data (including performance/quality metrics) into reporter
    reporter.processFileTrackingData(rawData.tests);
