  ...
  try {
    ...
    if (!options.skipTests) {
      ...
      if (!lineageDataExists()) {
        error(
          "Lineage data not found. Remove --skip-tests or run jest-lineage test first.",
        );
        process.exit(1);
      }
    ...
    if (!options.skipMutation) {
      ...
      try {
        ...
        error(
          "Mutation testing failed, but continuing with report generation...",
        );
      }
    } else {
    ...
    logger.info(chalk.gray(`  - .jest-lineage-data.json (lineage data)`));
    logger.info(chalk.gray(`  - ${options.output} (HTML report)`));
