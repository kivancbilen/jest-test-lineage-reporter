        ...
        inputSchema: {
          ...
          properties: {
            args: {
              ...
              items: { type: "string" },
              description:
                'Jest command-line arguments (e.g., ["--watch", "--testPathPattern=calculator"])',
              default: [],
            },
            enableLineage: {
              type: "boolean",
        ...
        inputSchema: {
          ...
          properties: {
            ...
            debug: {
              type: "boolean",
              description:
                "Create debug mutation files instead of running tests",
              default: false,
            },
          },
        },
    ...
    switch (name) {
              ...
              text: JSON.stringify(
                {
                  ...
                  meetsThreshold:
                    results.mutationScore >= (args.threshold || 80),
      ...
      case "query_coverage": {
        ...
        const matchingFiles = Object.keys(lineageData).filter(
          (f) =>
            f.includes(normalizedFile) ||
            normalizedFile.includes(path.basename(f)),
        );
...
async function main() {
  ...
  logger.info("Jest Test Lineage Reporter MCP server running on stdio");
}
